 ...
 */
@Component
public class TWSGameServices extends
    AbstractMultiPlayerGameServices<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> {

  private final SubmitFindHandler submitFindHandler;
  private final GiveHintHandler giveHintHandler;

  TWSGameServices(
      final GameGetterHandler<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> gameGetterHandler,
      final DeclineRematchOptionHandler<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> declineRematchOptionHandler,
      final ChallengeResponseHandler<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> responseHandler,
      final ChallengeToRematchHandler<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> rematchHandler,
      final QuitHandler<ObjectId, GameFeature, TWSGame, MaskedGame, MongoPlayer> quitHandler,
      final SubmitFindHandler submitFindHandler, final GiveHintHandler giveHintHandler) {
  ...
  public Object findWord(final List<GridCoordinate> relativeCoordinates) {
    return submitFindHandler
        .handleAction(getPlayerID().get(), getGameID().get(), relativeCoordinates);
  }
