...
public class TWSGameServicesTest extends MongoGameCoreTestCase {
    ...
    private TWSGameServices services = new TWSGameServices(null, null, null, null, null, submitFindHandler, giveHintHandler);

    @Test
    public void testActionAnnotations() {
        Map<String, List<Object>> stuff = new HashMap<>();
        stuff.put("findWord", Arrays.asList("find", Collections.singletonList(List.class), new ArrayList()));
        stuff.put("giveHint", Arrays.asList("hint", new ArrayList(), new ArrayList()));

        stuff.forEach((method, details) -> {
            final Method m;
            try {
                m = TWSGameServices.class.getMethod(method, ((List<Class>) details.get(1)).toArray(new Class[0]));
    ...
    public void testFindWordUsesHandler() {
        MaskedGame maskedGame = new MaskedGame();
        List<GridCoordinate> inputCoordinates = Arrays.asList(new GridCoordinate(0, 0), new GridCoordinate(1, 1), new GridCoordinate(1, 1));
        ObjectId gameId = new ObjectId();
        services.getPlayerID().set(PONE.getId());
        services.getGameID().set(gameId);
        when(submitFindHandler.handleAction(PONE.getId(), gameId, inputCoordinates)).thenReturn(maskedGame);
